...
public class ModifyProductFormController implements Initializable {
    ...
    Product tempProduct = new Product(99, "9",9,9.9,9,99);
    @FXML
...
//        createAssocPartsTable();
    ...
    public void assignAssocTemp() {
        assocPartList = assocPartsTable.getItems();
    }
    public void assignTempProduct() {
        tempProduct.setId(Integer.parseInt(idTextFieldProduct.getText()));
        tempProduct.setName(nameTextFieldProduct.getText());
        tempProduct.setStock(Integer.parseInt(invTextFieldProduct.getText()));
        tempProduct.setPrice(Double.parseDouble(priceTextFieldProduct.getText()));
        tempProduct.setMin(Integer.parseInt(minTextFieldProduct.getText()));
        tempProduct.setMax(Integer.parseInt(maxTextFieldProduct.getText()));

        assocPartList.forEach((assocPartList) -> {
            tempProduct.addAssociatedPart(assocPartList);
        });
    }
    ...
    private void createAssocPartsTable(ObservableList<Part> myList) {
        assocPartsTable.setItems(myList);
    ...
    public void fillProductFields(Product selectedPart) {
        idTextFieldProduct.setText(String.valueOf(selectedPart.getId()));
        nameTextFieldProduct.setText(selectedPart.getName());
        invTextFieldProduct.setText(String.valueOf(selectedPart.getStock()));
        priceTextFieldProduct.setText(String.valueOf(selectedPart.getPrice()));
        maxTextFieldProduct.setText(String.valueOf(selectedPart.getMax()));
        minTextFieldProduct.setText(String.valueOf(selectedPart.getMin()));

        ObservableList<Part> tempList = FXCollections.observableArrayList(selectedPart.getAllAssociatedParts());
        createAssocPartsTable(tempList);
        assignAssocTemp();
        assignTempProduct();
    }
